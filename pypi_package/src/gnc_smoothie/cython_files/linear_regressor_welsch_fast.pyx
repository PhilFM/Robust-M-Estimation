...
from cython.cimports.src.gnc_smoothie.cython_files.welsch_influence_func import welsch_rho, welsch_rhop, welsch_Bterm, welsch_rhopiv
from cython.cimports.src.gnc_smoothie.cython_files.linear_regressor_fast import linear_regressor_calc_residual, linear_regressor_calc_grad, increment_weighted_deriv_sums, increment_weighted_gnc_deriv_sums
...
def linear_regressor_welsch_weighted_gnc_derivs(cython.double sigma,
                                                cython.double[:] model,
                                                cython.double[:,:,:] data, cython.double[:] weight,
                                                cython.double[:] scale,
                                                cython.double[:] residual, cython.double[:,:] grad,
                                                cython.double[:] aivtot, cython.double[:,:] Atot):
    i: cython.Py_ssize_t
    for i in range(data.shape[0]):
        rsqr: cython.double = linear_regressor_calc_residual(model, data[i], residual)
        linear_regressor_calc_grad(data[i], residual, grad)
        ...
        rhop: cython.double = welsch_rhop(rsqr, inv_var)
        Bterm: cython.double = welsch_Bterm(rsqr, inv_var)
        rhopiv: cython.double = welsch_rhopiv(rsqr, inv_var)
        increment_weighted_gnc_deriv_sums(data[i], grad, rhop, rhopiv, Bterm, weight[i], aivtot, Atot)

    # fill in lower diagonal entries in Atot
    k: cython.Py_ssize_t
    l: cython.Py_ssize_t
    for k in range(data.shape[1]*data.shape[2]):
        for l in range(k+1,data.shape[1]*data.shape[2]):
