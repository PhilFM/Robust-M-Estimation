...
from cython.cimports.src.gnc_smoothie_philfm.cython.pseudo_huber_influence_func import pseudo_huber_rho, pseudo_huber_rhop, pseudo_huber_Bterm
...
def linear_regressor_pseudo_huber_objective_func(cython.double sigma,
                                                 cython.double[:] model,
    ...
    for i in range(data.shape[0]):
        ...
        tot += weight[i] * pseudo_huber_rho(rsqr, inv_var)

    return tot

def linear_regressor_pseudo_huber_weighted_derivs(cython.double sigma, cython.double lambda_val,
                                                  cython.double[:] model,
                                                  cython.double[:,:,:] data, cython.double[:] weight,
                                                  cython.double[:] scale,
    ...
    for i in range(data.shape[0]):
        ...
        this_sigma: cython.double = scale[i] * sigma
        inv_var: cython.double = 1.0 / (this_sigma * this_sigma)
        rhop: cython.double = pseudo_huber_rhop(rsqr, inv_var)
        Bterm: cython.double = pseudo_huber_Bterm(rsqr, inv_var)
...
def linear_regressor_pseudo_huber_update_weights(cython.double sigma,
                                                 cython.double[:] model,
                                                 cython.double[:,:,:] data, cython.double[:] weight,
                                                 cython.double[:] scale,
    ...
    for i in range(data.shape[0]):
        ...
        this_sigma: cython.double = scale[i] * sigma
        inv_var: cython.double = 1.0 / (this_sigma * this_sigma)
        new_weight[i] = weight[i]*pseudo_huber_rhop(rsqr, inv_var)
