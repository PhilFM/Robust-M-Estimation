...
class LineFitWelsch:
    ...
    def __convert_model(self, model: np.array) -> np.array:
        return np.array([model[0], -1.0, model[1]])/math.sqrt(1.0+model[0]*model[0])
    ...
    def run(self,
        ...
        optimiser_instance = SupGaussNewton(param_instance, LineFit(), data, weight=weight, scale=scale,
        ...
        if optimiser_instance.run():
            ...
            if self.__debug:
                self.debug_line_list = [(model[0], self.__convert_model(model[1])) for model in optimiser_instance.debug_model_list]
