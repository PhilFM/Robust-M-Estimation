...
def mean_welsch_solver(data: np.array, scale: np.array, x_range: float, sigma_pop: float,
    ...
    irls_instance = IRLS(param_instance, model_instance, data, max_niterations=max_niterations, print_warnings=False)
    if irls_instance.run():
        m = irls_instance.final_model[0]
        if not test_run:
            print("Welsch IRLS result: m=", m)

    mean_finder = MeanWelsch(sigma_base, sigma_limit=sigma_limit, num_sigma_steps=num_sigma_steps,
                             max_niterations=max_niterations, print_warnings=False, debug=True)
    if mean_finder.run(data):
        m = mean_finder.final_mean
        if not test_run:
            print("Welsch Sup-GN optimisation result: m=", m)
            print("  final weights:",mean_finder.final_weight)

    # check result when scale is included
    if mean_finder.run(data, scale=scale):
        mscale = mean_finder.final_mean
        if not test_run:
            print("Welsch scale result difference=", mscale-m)

    sup_gn_instance = SupGaussNewton(param_instance, model_instance, data, scale=scale,
                                     max_niterations=max_niterations, print_warnings=False)
    ...
    last_xy = None
    last_color = None
    for alpha in np.linspace(1.0, 0.1, num=10):
        sup_gn_instance._param_instance.influence_func_instance.sigma = sigma_base * math.pow(sigma_limit/sigma_base, alpha)
        color = (0.8*alpha, 1.0, 0.0)
        plt.plot(mlist, hmfv(mlist, optimiser_instance=sup_gn_instance), lw = 1.0, color=color)

        # mark the maximum
        idx = np.argmax([ objective_func(mx, sup_gn_instance) for mx in mlist])
        mx = mlist[idx]
        this_xy = (mx, objective_func(mx, sup_gn_instance))
    ...
    sup_gn_instance._param_instance.influence_func_instance.sigma = sigma_base
    plt.plot(mlist, hmfv(mlist, optimiser_instance=sup_gn_instance), lw = 1.0, color="green", label="Welsch objective function")
    ...
    # add invisible line just for legend
    alpha = 0.5
    color = (0.8*alpha, 1.0, 0.0)
    plt.axvline(x = -1000, color = color, ymax = 0.1, lw = 1.0, label="GNC intermediates")
    
    for d in data:
        plt.axvline(x = d, color = 'b', ymax = 0.1, lw = 1.0)

    plt.axvline(x = d[0], color = 'b', ymax = 0.1, lw = 1.0, label="Data values")

    plt.axvline(x = m, color = 'r', label = 'Estimated mean', lw = 1.0)

    plt.legend()
    plt.savefig(os.path.join(output_folder, "mean_welsch.png"), bbox_inches='tight')
    if not test_run:
        plt.show()
...
def mean_pseudo_huber_solver(data: np.array, scale: np.array, x_range: float, sigma_pop: float,
                             test_run: bool, output_folder: str) -> None:
    model_instance = RobustMean()
    influence_func_instance = PseudoHuberInfluenceFunc(sigma=sigma_pop)
    param_instance = GNC_NullParams(influence_func_instance)
    irls_instance = IRLS(param_instance, model_instance, data, max_niterations=200, print_warnings=False)
    if irls_instance.run():
        m = irls_instance.final_model[0]
        if not test_run:
    ...
    # for graph plotting
    sup_gn_instance = SupGaussNewton(param_instance, model_instance, data, print_warnings=False)
    irls_instance = IRLS(GNC_NullParams(influence_func_instance), model_instance, data, scale=scale)
    if irls_instance.run():
...
def mean_geman_mcclure_solver(data: np.array, scale: np.array, x_range: float, sigma_pop: float,
    ...
    irls_instance = IRLS(param_instance, model_instance, data, print_warnings=False)
    if irls_instance.run():
        m = irls_instance.final_model[0]
        if not test_run:
    ...
    irls_instance = IRLS(param_instance, model_instance, data, scale=scale)
    if irls_instance.run():
        mscale = irls_instance.final_model[0]
        if not test_run:
    ...
    hmfv = np.vectorize(objective_func, excluded={"optimiser_instance"})
    plt.plot(mlist, hmfv(mlist, optimiser_instance=sup_gn_instance), lw = 1.0, color="g", label="Geman-McClure objective function")
    for d in data:
        plt.axvline(x = d, color = 'b', ymax = 0.1, lw = 1.0)
    ...
    plt.legend()
    plt.savefig(os.path.join(output_folder, "mean_geman_mcclure.png"), bbox_inches='tight')
    if not test_run:
        plt.show()
...
def mean_gnc_irls_p_solver(data: np.array, scale: np.array, x_range: float, sigma_pop: float,
    ...
    # for graph plotting
    sup_gn_instance = SupGaussNewton(param_instance, model_instance, data, print_warnings=True)
    ...
    hmfv = np.vectorize(objective_func, excluded={"optimiser_instance"})
    plt.plot(mlist, hmfv(mlist, optimiser_instance=sup_gn_instance), lw = 1.0, color="g", label="GNC IRLS-p0 objective function")
    for d in data:
        plt.axvline(x = d, color = 'b', ymax = 0.1, lw = 1.0)
    ...
    plt.legend()
    plt.savefig(os.path.join(output_folder, "mean_gnc_irls_p0.png"), bbox_inches='tight')
    if not test_run:
        plt.show()
