...
def test_with_sigma(line_gt, data_x, data_y, sigma: float, output_folder: str, test_run):
    ...
    line_fitter = LinearRegressorWelsch(sigma_base=sigma, sigma_limit=y_range, num_sigma_steps=10,
                                        debug=False if test_run else True, max_niterations=200, messages_file=None if test_run else sys.stdout)
    ...
    if test_orthogonal_regression:
        ...
        line_fitter_orthog = LineFitOrthogWelsch(sigma, sigma_limit=y_range, debug=True)
        data_xp = np.reshape(data_x, (len(data_x),1))
        data_yp = np.reshape(data_y, (len(data_y),1))
        if line_fitter_orthog.fit(np.concatenate((data_xp, data_yp), axis=1)):
    ...
    plt.legend()
    plt.savefig(os.path.join(output_folder, "example.png"), bbox_inches='tight')
    if not test_run:
        plt.show()
    ...
    if test_orthogonal_regression:
        ...
        plt.legend()
        plt.savefig(os.path.join(output_folder, "example_orthog.png"), bbox_inches='tight')
        if not test_run:
            plt.show()
...
def test_with_sigma_pseudo_huber(line_gt, data_x, data_y, sigma: float, output_folder: str, test_run: bool):
    ...
    if line_fitter.fit((data_x, data_y), model_start=[randomM11(),randomM11()]):
        coeff = line_fitter.final_coeff
        intercept = line_fitter.final_intercept
        final_line = np.array([coeff[0][0], intercept[0]])
        final_weight = line_fitter.final_weight
    ...
    plt.legend()
    plt.savefig(os.path.join(output_folder, "example_pseudo_huber.png"), bbox_inches='tight')
    if not test_run:
        plt.show()
