...
if __name__ == "__main__":
    ...
    # now get the SupGN result
    y_range = max(y) - min(y)
    sigma = 0.1
    line_fitter = LinearRegressorWelsch(sigma_base=sigma, sigma_limit=y_range, num_sigma_steps=10,
                                        debug=True, max_niterations=200, messages_file=sys.stdout)
    if line_fitter.fit((X, y)):
        ...
        x_min = min(X)[0]
        x_max = max(X)[0]
        print("x_minmax",x_min,x_max)
        (a,b) = (coeff[0][0], intercept[0])
        print("ab=",a,b)
        plt.axline((x_min, a*x_min+b), (x_max, a*x_max+b), linewidth=1.0)
        plt.show()
