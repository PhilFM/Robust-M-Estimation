...
def main(test_idx:int = -1, test_run:bool=False, output_folder:str="../../../output", quick_run:bool=False):
    ...
    n_samples_base = 0 #: 100 if quick_run else 100
    min_n_samples = 2 if quick_run else 5

    np.random.seed(0) # We want the numbers to be the same on each run

    n_dimensions_list = [2,5] if quick_run else np.linspace(2,20,10,dtype=int)
    sample_size_array = np.linspace(30,90,3,dtype=int) if quick_run else np.linspace(1000,10000,10,dtype=int)
    ...
    for ndim in n_dimensions_list:
        ...
        for n_data_points in sample_size_array:
            ...
            # GNC IRLS Welsch with scale estimation
            if False:
        ...
        n_values = len(sample_size_array)
        if False:
            plt.close("all")
            plt.figure(num=1, dpi=240)
            plt.clf()
            ax = plt.gca()
            gncs_draw_curve(plt, eff_gnc_welsch_list,    ("SupGN",     "Welsch",      "SS"         ), xvalues=sample_size_array)
            gncs_draw_curve(plt, eff_huber_list,         ("SupGN",     "PseudoHuber", "PseudoHuber"), xvalues=sample_size_array)
            gncs_draw_curve(plt, eff_gnc_irls_p_list,    ("SupGN",     "GNC_IRLSp",   "GNC_IRLSp0" ), xvalues=sample_size_array)

            ax.set_xlabel(r'Sample size' )
            ax.set_ylabel('Relative efficiency')
            ax.set_xlim(0.0,sample_size_array[n_values-1])
            ax.set_ylim(0.0,1.1)

            plt.legend()
            plt.savefig(os.path.join(output_folder, "ssize_efficiency_known_scale_ndim" + str(ndim) + ".png"), bbox_inches='tight')
        ...
        plt.clf()
        ax = plt.gca()
        gncs_draw_curve(plt, av_time_gnc_welsch_list,    ("SupGN",     "Welsch",      "SS"         ), xvalues=sample_size_array)
        gncs_draw_curve(plt, av_time_huber_list,         ("SupGN",     "PseudoHuber", "PseudoHuber"), xvalues=sample_size_array)
        gncs_draw_curve(plt, av_time_gnc_irls_p_list,    ("SupGN",     "GNC_IRLSp",   "GNC_IRLSp0" ), xvalues=sample_size_array)
        ...
        plt.legend()
        plt.savefig(os.path.join(output_folder, "ssize_av_time_known_scale_ndim" + str(ndim) + ".png"), bbox_inches='tight')
        if False: #not test_run:
            plt.show()
        ...
        if False:
            ...
            ax = plt.gca()
            gncs_draw_curve(plt, eff_gnc_welsch_list,     ("SupGN",         "Welsch",         "SS"       ), xvalues=sample_size_array)
            gncs_draw_curve(plt, eff_mm_estimation_list,  ("MM-Estimation", "Tukey-Bisquare", "Large-Rho"), xvalues=sample_size_array)
            gncs_draw_curve(plt, eff_theil_sen_list,      ("Theil-Sen",     "",               ""         ), xvalues=sample_size_array)
            gncs_draw_curve(plt, eff_ransac_list,         ("RANSAC",        "",               ""         ), xvalues=sample_size_array)
            ...
            plt.legend()
            plt.savefig(os.path.join(output_folder, "ssize_efficiency_ndim" + str(ndim) + ".png"), bbox_inches='tight')
            if False: #not test_run:
                plt.show()
        ...
        ax = plt.gca()
        gncs_draw_curve(plt, av_time_gnc_welsch_list,     ("SupGN",         "Welsch",         "SS"       ), xvalues=sample_size_array)
        gncs_draw_curve(plt, av_time_mm_estimation_list,  ("MM-Estimation", "Tukey-Bisquare", "Large-Rho"), xvalues=sample_size_array)
        gncs_draw_curve(plt, av_time_theil_sen_list,      ("Theil-Sen",     "",               ""         ), xvalues=sample_size_array)
        gncs_draw_curve(plt, av_time_ransac_list,         ("RANSAC",        "",               ""         ), xvalues=sample_size_array)
        #gncs_draw_curve(plt, av_time_ls_list,             ("LS",            "",               ""         ), xvalues=sample_size_array)
        ...
        plt.legend()
        plt.savefig(os.path.join(output_folder, "ssize_av_time_ndim" + str(ndim) + ".png"), bbox_inches='tight')
        if False: #not test_run:
            plt.show()
        ...
        plt.legend()
        plt.savefig(os.path.join(output_folder, "ssize_av_time_gnc_welsch_ndim" + str(ndim) + ".png"), bbox_inches='tight')
        if False: #not test_run:
            plt.show()
        ...
        plt.legend()
        plt.savefig(os.path.join(output_folder, "ssize_av_time_mm_estimation_ndim" + str(ndim) + ".png"), bbox_inches='tight')
        if False: #not test_run:
            plt.show()
        ...
        plt.legend()
        plt.savefig(os.path.join(output_folder, "ssize_av_time_theil_sen_ndim" + str(ndim) + ".png"), bbox_inches='tight')
        if False: #not test_run:
            plt.show()
        ...
        plt.legend()
        plt.savefig(os.path.join(output_folder, "ssize_av_time_ransac_ndim" + str(ndim) + ".png"), bbox_inches='tight')
        if False: #not test_run:
            plt.show()

    if test_run:
        print("regression_complexity_sample_size OK")
