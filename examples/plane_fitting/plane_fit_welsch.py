...
class PlaneFitWelsch:
    ...
    def __convert_model(self, model: np.array) -> np.array:
        return np.array([model[0], model[1], -1.0, model[2]])/math.sqrt(1.0+model[0]*model[0]+model[1]*model[1])
    ...
    def run(self,
        ...
        optimiser_instance = SupGaussNewton(param_instance, PlaneFit(), data, weight=weight, scale=scale,
